...
## Real-World Impact: This Machine's Own VS Code Extension 2026-08-06

User reported the Markplane VS Code extension (all open windows) stuck on "Project Graph is starting", start/restart buttons doing nothing, and `markplane check` failing with "Der Befehl 'markplane' wurde nicht erkannt". Investigated and found two independent, pre-existing problems (neither caused by this session's git/CI work — verified no files under `%LOCALAPPDATA%\Programs\Markplane`, no VS Code settings, and no PATH entry were touched before this):

1. `packages/vscode-extension/source/extension.js` resolves the binary via `vscode.workspace.getConfiguration("markplane").get("executable", "markplane")` — an unconfigured install falls back to the bare command `markplane`, resolved via PATH. Confirmed via `[Environment]::GetEnvironmentVariable("PATH","User"/"Machine")` (registry, not just shell snapshot) that neither `%LOCALAPPDATA%\Programs\Markplane` nor `.cargo\bin` has ever been on PATH, and no `markplane.executable` setting existed in the user's global `settings.json` — so bare `markplane` could never resolve.
2. Even with PATH fixed, the actually-installed `%LOCALAPPDATA%\Programs\Markplane\markplane.exe` (installed 2026-08-04, confirmed unchanged by a local installer re-run the user did today) lacked `embed-ui` — same 404 bug as the colleague report above, just on this machine's real install.

User confirmed mid-session they ran the local installer again today, which reinstalled `%LOCALAPPDATA%\Programs\Markplane\` and the VS Code extension folder but repackaged the still-broken (pre-existing, non-embed-ui) `installer\windows\markplane.exe`, byte-for-byte reproducing problem 2. User then explicitly asked for "our modified version of markplane" (the current `components/markplane` fork) to be what actually runs everywhere.

Immediate fix (before rebuilding): added `"markplane.executable": "C:\\Users\\scheurer\\.cargo\\bin\\markplane.exe"` to `%APPDATA%\Code\User\settings.json`, since that copy already had embed-ui — unblocks the extension in every window via a VS Code reload, independent of PATH.

Full fix: rebuilt `components\markplane` fresh with `--features embed-ui`, verified (200 on `/` and `/graph`, "Serving embedded UI from binary"), then deployed the identical binary (same SHA256 `a05f8f3a...`) to all three places a `markplane.exe` matters on this machine:
- `installer\windows\markplane.exe` (repo dev file, gitignored/untracked — feeds local and CI installer builds)
- `%LOCALAPPDATA%\Programs\Markplane\markplane.exe` (the real installed program; Claude/Codex/Antigravity hooks reference this by absolute path per `installer/windows/README.md`)
- `C:\Users\scheurer\.cargo\bin\markplane.exe` (backs this session's own Markplane MCP server — found it running as `markplane.exe mcp`, PID confirmed via `Get-CimInstance Win32_Process`, but its actual registration isn't in this repo's `.mcp.json` (doesn't exist) or the global VS Code `mcp.json` (empty), so it's stored in some VS Code-extension-internal state not worth hunting down mid-session; simplest safe fix was keeping this copy in sync rather than repointing hidden config)

The `.cargo\bin` copy was locked by the running MCP server process; used the rename-then-copy trick (`mv markplane.exe markplane.exe.old-<timestamp>`, then copy the new build in) rather than killing the process — Windows allows renaming an open-for-execute file, and the old process kept running fine on its already-open handle throughout. Left `markplane.exe.old-20260806-113701` as a safety-net backup alongside the pre-existing `markplane.exe.markplane-pre-project-lock.bak`.

Also fixed a real bug in the new `MarkplaneWebUi.Tests.ps1`: `$process.Kill()` doesn't wait for the process to actually exit, so Pester's end-of-`Describe` `Remove-TestDrive` could race a still-live `markplane serve` process holding handles under `$TestDrive`. Added a `Stop-MarkplaneServe` helper that calls `WaitForExit(5000)` after `Kill()`. Note: this machine's PowerShell session is independently hitting an unrelated, pre-existing `Remove-TestDrive: Zugriff verweigert` (Access Denied) flake after many repeated Pester runs today — reproduced identically on a completely untouched test file (`RwcaProvenance.Tests.ps1`), so it's an environmental/TEMP-directory issue with this long-running session, not something introduced by this work. The actual test assertions (both in `MarkplaneWebUi.Tests.ps1` and the untouched file) pass reliably every time regardless.
