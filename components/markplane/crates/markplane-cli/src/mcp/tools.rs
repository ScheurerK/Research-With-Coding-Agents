            ...
            },
            {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        "focus": {
                            "type": "string",
                            "description": "Optional focus area (e.g. 'active-work', 'blocked', 'metrics', 'resume')"
                        }
                    },
                    "required": []
                }
...
fn handle_context(project: &Project, args: &Value) -> Result<String, String> {
    ...
    let (generate, filename): (fn(&Project) -> markplane_core::Result<()>, &str) = match focus {
        ...
        Some("resume") => (Project::generate_context_resume, "resume.md"),
        None | Some("summary") => (Project::generate_context_summary, "summary.md"),
        Some(other) => {
            return Err(format!(
                "Unknown focus area: {}. Expected active-work, blocked, metrics, resume, or summary",
                other
            ));
        }
    };
