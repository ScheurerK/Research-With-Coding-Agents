...
| `markplane_context` | Generate a context summary for the project. | *(none)* | `focus` (string): area — `summary` (default), `active-work`, `blocked`, `metrics`, or `resume` (lean reorientation view, see [AI Integration Guide](ai-integration.md#resumemd)) |
